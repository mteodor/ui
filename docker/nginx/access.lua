...
local opts = {
 -- the full redirect URI must be protected by this script
   ...
   discovery = "http://172.18.0.25:8080/auth/realms/master/.well-known/openid-configuration",
   client_id = "openresty",
   client_secret = "85740c02-124a-4160-97cf-a63df08ce525",
   token_endpoint_auth_method = "client_secret_post",
   scope = "openid email profile",
   introspection_endpoint = "http://172.18.0.25:8080/auth/realms/master/protocol/openid-connect/token/introspect",
   introspection_endpoint_auth_method = "client_secret_post",
   redirect_uri_scheme = "http",
   logout_path = "/logout",
   redirect_after_logout_uri = "http://172.18.0.25:8080/auth/realms/master/protocol/openid-connect/logout?redirect_uri=http://www.example.com/",
...
ngx.log(ngx.ERR, "Response:", payload)
ngx.log(ngx.ERR, "email:", res.email)
ngx.log(ngx.ERR, "access_token:", res.access_token)
